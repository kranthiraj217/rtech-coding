<?php
define('g', 9.8); //global variable for gravity

//Data of the Dinosaurus
...
//loop for calculating speed of each reptile
for($i=0;$i<=5;$i++){
    $speed = (($STRIDE_LENGTH[$i] / $LEG_LENGTH[$i] - 1) * sqrt($LEG_LENGTH[$i] * g));

}
//loop for displaying the reptiles in O(fastest to slowest)
for($i=5; $i>=0 && $i<=5 ;$i--){
